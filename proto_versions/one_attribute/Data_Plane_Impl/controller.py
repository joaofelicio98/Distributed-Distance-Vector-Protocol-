...
class Controller():
    ...
    def recv_msg_cpu(self, pkt):
        ...
        if pkt[IP].proto == CPU_HEADER_PROTO:
            ...
            print(f"DEBUG {self.sw_name} | destination = {dst_ip}")
            print(f"DEBUG {self.sw_name} | next hop = {port}")
            print(f"DEBUG {self.sw_name} | is_new = {is_new}")

            if is_new == 1:
                self.add_new_entry(dst_ip, port)
            else:
                self.modify_entry(dst_ip, port)
    ...
    def add_new_entry(self, dst_ip, port):
        ...
        self.controller.table_add("ipv4_lpm", "ipv4_forward",
                                [dst_ip], [dst_mac, str(port)])
        self.controller.table_add("check_destination_known", "get_registers_info",
                                [dst_ip])
